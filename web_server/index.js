...
app.get("/api/surveys/:id", (req, res, next) => {
    req.dataFindByIdAndSend = {data:data.surveys, errorMsg : "Survey was not found"};
    next();
}, findByIdAndSend)

app.post("/api/surveys/create", (req, res) => {
    const {name, dtcreation, dtend, survey:{question, answers}} = req.body;
    ...
    if(!Number.isInteger(dtcreation) || !Number.isInteger(dtend)){
        res.send({error:"Creation date or End date of the survey are invalid, we expected an integer js timestamp."});
        return;
    }
    const id = uuid();
    const newSurvey = {name, dtcreation, dtend, survey:{question, answers}, id, data:[]};
...
app.post("/api/surveys/update/:id", (req, res) => {
    ...
    data.surveys = data.surveys.map(survey=>{
        if (survey.id == id && !foundSurvey){
            survey = {...survey, name, dtcreation, dtend, survey:{question, answers}};
            foundSurvey = survey;
        }
        return survey;
    });
